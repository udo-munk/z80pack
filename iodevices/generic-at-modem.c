...
static int hangup_timeout(bool start)
{
	...
	if (*active_sfd) {
		if (start) {
			...
			hup_t2 = get_clock_us();
			/* scale usec to 10ths of seconds */
			tdiff = (hup_t2 - hup_t1) / 100000;
			/* SREG_HUP_DELAY is in 10ths of seconds */
			if (tdiff >= (int) (s_reg[SREG_HUP_DELAY])) {
				waiting = 0;
				s_reg[SREG_RINGS] = 0;
				close_socket();
...
static int answer(void)
{
	...
	if (s_reg[SREG_TELNET]) {
		...
		telnet_negotiate(telnet, (s_reg[SREG_TN_SGA] & 1) ? TELNET_WILL : TELNET_WONT,
				 TELNET_TELOPT_SGA);
		telnet_negotiate(telnet, (s_reg[SREG_TN_ECHO] & 1) ? TELNET_WILL : TELNET_WONT,
				 TELNET_TELOPT_ECHO);
	};
...
typedef enum at_state { cmd, A_recv, AT_recv, AS_recv, dat, intr, help } at_state_t;
...
static const char *at_help[] = {
	...
	CRLF,
#endif
	"ATDhostname:port - Dial hostname, port optional (default:"
	STR_VALUE(DEFAULT_LISTENER_PORT) ")" CRLF,
	...
	while (*at_ptr != 0) {
		...
		case 'I':
			...
			if (tmp_reg == 0) {
				...
				sprintf(at_err, LF "DMA %d" CRLF,
					heap_caps_get_free_size(MALLOC_CAP_DMA));
				at_cat_s(at_err);
			} else if (tmp_reg == 5) {
				sprintf(at_err, LF "INTERNAL %d" CRLF,
					heap_caps_get_free_size(MALLOC_CAP_INTERNAL));
				at_cat_s(at_err);
			} else if (tmp_reg == 6) {
				sprintf(at_err, LF "TOTAL %d" CRLF,
				...
				/* Writes to FTDI UART not the modem */
		...
		case '&':
			if (*at_ptr == 'A') { /* AT&A - enable answer - listen */
				...
				if (tmp_reg) {
					...
					s_reg[SREG_OPT] &= ~OPT_ECHO;
					s_reg[SREG_OPT] |= OPT_QUIET;
					LOGI(TAG, "MODEM will be daemon and listen "
					     "(No Echo & Quiet)");
				}
				break;
	...
	case intr:
		if (data == '+' && strlen(at_buf) < 3) {
			...
			if (telnet != NULL)
				...
				if (write(*active_sfd, at_buf, strlen(at_buf))
