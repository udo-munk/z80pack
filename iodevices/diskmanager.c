		...
		if (image != NULL && strlen(image) < MAX_LFN) {
			...
			if (stat(path, &image_status) == 0) {
				if (S_ISREG(image_status.st_mode)) {
					...
					if (name == NULL) {
						LOGW(TAG, "Failed to insert disk, "
						     "not enough memory.");
						return FAILURE;
					} else {
				...
				LOGW(TAG, "Failed to stat disk image: %s, error: %d",
				     image, errno);
				return IMAGE_NOT_VALID;
			}
		}
		...
		if (res != -1) {
			...
			if (name != NULL) {
				...
				case IMAGE_ALREADY_INSERTED :
					LOGW(TAG, "%c:DSK: Image file '%s' already in use",
					     i + 'A', name);
					break;
				case IMAGE_NOT_VALID :
					LOGW(TAG, "%c:DSK: Image file '%s' is invalid",
					     i + 'A', name);
					break;
				default:
					LOGW(TAG, "%c:DSK: Failed to insert disk, error: %d",
					     i + 'A', insert);
					break;
				}
			}
		...
		if (req->len > 0) {
			...
			*(file_start + i) = '\0';
			...
			if (findDiskImage(file_start)) {
				LOGW(TAG, "DELETE image: %s, currently inserted in disks",
				     file_start);
...
static void sendDisks(HttpdConnection_t *conn)
{
	int i;
...
int DiskHandler(HttpdConnection_t *conn, void *unused)
{
	...
	case HTTP_PUT:
		...
		case IMAGE_ALREADY_INSERTED:
			...
			break;
		default:
			LOGW(TAG, "PUT image: %s, failed to insert disk: %d, error: %d",
			     image, disk, result);
			httpdStartResponse(conn, 404);  /* http error code 'Not Found' */
			httpdEndHeaders(conn);
			break;
	...
	case HTTP_DELETE:
		...
		case DRIVE_EMPTY:
			...
			break;
		default:
			LOGW(TAG, "DELETE /disks failed to eject disk: %d, error: %d",
			     disk, result);
			httpdStartResponse(conn, 404);	/* http error code 'Not Found' */
			httpdEndHeaders(conn);
			break;
