...
ONCPM	EQU	1	;1 = RUNS ON CPM, 0 = BARE EMULATOR/MACHINE
	ENDIF
IOCHAR	EQU	01H	;I/O PORT FOR CHARACTER OUTPUT (BARE)
...
START:	XRA	A
		STA	SWFLAG		; CLEAR CPU SWITCHED FLAG
		DCR	A
		JPE	START1		; EVEN PARITY -> IS 8080
		CALL	SW8080		; SWITCH TO 8080
		MVI	A,0DH
		CALL	PCHAR
		MVI	A,0AH
		CALL	PCHAR
...
;
LOLZ:	DB	'MICROCOSM ASSOCIATES 8080/8085'
	DB	' CPU DIAGNOSTIC VERSION 1.0  (C) 1980'
	DB	0dh, 0ah, 24h
;
BDOS	EQU		00005H	;BDOS ENTRY TO CP/M
WBOOT:	LDA	SWFLAG	; CHECK CPU SWITCHED FLAG
		ORA	A
		JZ	WBOOT1
		MVI	A,0DH
		CALL	PCHAR
...
SWZ80:	MVI	A,32
		LXI	H,SZ80
DOSW:	MOV	C,A	; SAVE CPU TYPE
		IN	0A0H	; CHECK IF HARDWARE CONTROL PORT IS UNLOCKED
		ORA	A
		JZ	DOSW1
		MVI	A,0AAH	; UNLOCK HARDWARE CONTROL PORT
		OUT	0A0H
DOSW1:	MOV	A,C		; SWITCH CPU TYPE
		OUT	0A0H
		STA	SWFLAG	; SET CPU SWITCHED FLAG
		JMP	MSG

SWFLAG:	DS	1
SZ80:	DB	'Switched to Z80 processor$'
S8080:	DB	'Switched to 8080 processor$'
...
;
		...
		MVI	D,046H
		MVI	E,047H
		MVI	H,HIGH TEMP0	;HIGH BYTE OF TEST MEMORY LOCATION
		MVI	L,LOW TEMP0	;LOW BYTE OF TEST MEMORY LOCATION
		MOV	M,B
		MVI	B,000H
		MOV	B,M
		MVI	A,044H
