	...
	case HTTP_GET:
		...
		for (int i = 0; i < MAXMEMMAP; i++) {
			if (memconf[M_value][i].size) {
				if (i > 0)
					httpdPrintf(conn, ", ");
				httpdPrintf(conn, "{ \"type\": \"%s\"",
					    (memconf[M_value][i].type == MEM_RW) ? "RAM" : "ROM");
				httpdPrintf(conn, ", \"from\": %d",
					    memconf[M_value][i].spage << 8);
				httpdPrintf(conn, ", \"to\": %d",
					    ((memconf[M_value][i].spage << 8)
					     + (memconf[M_value][i].size << 8) - 1));
				if (memconf[M_value][i].type == MEM_RO
				    && memconf[M_value][i].rom_file)
					httpdPrintf(conn, ", \"file\": \"%s\"",
						    memconf[M_value][i].rom_file);
				httpdPrintf(conn, "}");
			}
		}
...
#define BULLET  "\xE2\x80\xA2"
				if (!strcmp(t1, "PASSWORD")
				    && (getenv("WIFI.password.hide") != NULL))
					t2 = BULLET BULLET BULLET BULLET
	...
	case HTTP_GET:
		...
		if (pDir == NULL) {
			...
			while ((pDirent = readdir(pDir)) != NULL) {
				LOGD(TAG, "GET directory: %s type: %d",
				     pDirent->d_name, pDirent->d_type);
				snprintf(fullpath, MAX_LFN, "%s/%s",
					 (char *) path, pDirent->d_name);
				/*
				 * not working with some filesystems like Linux xfs,
				 * need to use stat()
				 * if (pDirent->d_type==DT_REG) {
				 */
				if (stat(fullpath, &sb) != -1 && S_ISREG(sb.st_mode)) {
					if (showSize) {
						httpdPrintf(conn, "%c{ \"%s\":\"%s\",",
							    (i++ > 0) ? ',' : ' ',
							    "filename", pDirent->d_name);
						httpdPrintf(conn, "\"%s\":%lld}", "size",
							    (long long) sb.st_size);
					} else
						httpdPrintf(conn, "%c\"%s\"",
							    (i++ > 0) ? ',' : ' ',
							    pDirent->d_name);
				}
			}
...
static int WebsocketDataHandler(HttpdConnection_t *conn,
				int bits, char *data, size_t len, void *device)
{
	net_device_t d = *(net_device_t *) device;
	...
	if ((((unsigned char) bits) & 0x0F) == MG_WEBSOCKET_OPCODE_BINARY) {
		...
		case DEV_PTR:
			if (len != 1) {
				LOGW(TAG, "Websocket received too many [%d] characters",
				     (int) len);
				return 0;
			}
			msg.mtype = 1L;
...
int start_net_services(int port)
{
	...
	const char *options[] = {
		...
		"/" MACHINE "/disks/=./disks/, /" MACHINE "/conf/=./conf/, /"
		MACHINE "/printer.txt=./printer.txt",
		0
	};
